...
FixMeso::FixMeso(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {
  printf ("Fix being called\n");

  if ((atom->e_flag != 1) || (atom->rho_flag != 1) || (atom->prtn_conc_flag != 1))
    error->all(FLERR,
        "fix meso command requires atom_style with both energy and density and proton concentration");
...
void FixMeso::initial_integrate(int vflag) {
  ...
  for (i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      //if (type[i] != 1) 
        prtn_conc[i] += dtf * dprtn_conc[i]; // .. and proton concentration
        q[i] += dtf*dq[i];
      ...
      v[i][1] += dtfm * f[i][1];
      v[i][2] += dtfm * f[i][2];
//printf ("Previous x %e ",x[i][0]);
      x[i][0] += dtv * v[i][0];
//printf ("After x %e __ vx %e dtv %e\n",v[i][1],dtv); 
      x[i][1] += dtv * v[i][1];
      x[i][2] += dtv * v[i][2];
    }
  }
...
void FixMeso::final_integrate() {
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      rho[i] += dtf * drho[i];
      //if (type[i] != 1)
        prtn_conc[i] += dtf * dprtn_conc[i];
//        if(abs(dprtn_conc[i])>1000000)
//           printf("Abnormal change in prtn_conc for atom %d, and dtf*dprtn_conc = %e\n",i,dtf*dprtn_conc[i]);
        q[i] += dtf*dq[i]; 
    }
  }
}
